IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SPR_ICM_LISTA_SUB_PROCESSO_DIRETO_POR_HOMOLOGACAO]') AND type in (N'P', N'PC'))
begin
    EXEC('CREATE PROCEDURE [dbo].[SPR_ICM_LISTA_SUB_PROCESSO_DIRETO_POR_HOMOLOGACAO] AS BEGIN SET NOCOUNT ON; END');
    GRANT EXECUTE ON dbo.[SPR_ICM_LISTA_SUB_PROCESSO_DIRETO_POR_HOMOLOGACAO] to LogCacMv
end
GO

ALTER PROCEDURE dbo.[SPR_ICM_LISTA_SUB_PROCESSO_DIRETO_POR_HOMOLOGACAO](
...
BEGIN
    ...
    from    SUB_PROCESSO sp inner join PROCESSO pp
        ...
        inner join ICM_SUB_PROCESSO_DIRETO spd
            on sp.ano_apresentacao = spd.ano_apresentacao
            and sp.id_representacao = spd.id_representacao
            and sp.id_processo = spd.id_processo
            and sp.d_sub_processo = spd.d_sub_processo
            and sp.d_natureza = spd.d_natureza
            and sp.id_sequencial_natureza = spd.id_sequencial_natureza            
